...
client.on("message", (msg) => {
  if (msg.content.includes("<@!1090328401556029480>"))
    return msg.delete() && msg.channel.send(`Etiketlemek yasak!!!`);
});

client.on("userUpdate", async (oldUser, newUser) => {
  if (oldUser.username !== newUser.username) {
    let tag = "♅"; //tagınız
    let sunucu = "1116793508469149707"; //sunucu ID
    let kanal = "1116808776671043767"; //log kanal id
    let rol = "1116808651064221807"; // rol ID
    if (
      ...
      client.channels.cache
        .get(kanal)
        .send(
          `${newUser} isimli kullanıcı kullanıcı adına tagımızı eklediği için rol ve ayrıcalıkları verilmiştir.`
        );
      client.guilds.cache
        .get(sunucu)
        .members.cache.get(newUser.id)
    ...
    if (
      ...
      client.channels.cache
        .get(kanal)
        .send(
          `${newUser} isimli kullanıcı kullanıcı adından tagı çıkardığı için ayrıcılıkları ve rolü alınmıştır.`
        );
    }
  }
});
