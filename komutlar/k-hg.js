...
exports.run = async (client, message, args) => {
  ...
  if (args[0] === "sıfırla") {
    const sıfırlandı = new discord.MessageEmbed()
      ...
      .setTitle(
        `${client.user.username} | Hoş geldin kanalını sıfırlama komutu.`
      )
      .setColor(0x36393f)
  ...
  if (!kanal) {
    const ayarlanmadı = new discord.MessageEmbed()
      .setAuthor(client.user.username, client.user.avatarURL())
      .setTitle(
        `${client.user.username} | Hoş geldin kanalını ayarlama komutu.`
      )
      .setColor(0x36393f)
  ...
  const ayarlandı = new discord.MessageEmbed()
    ...
    .setColor(0x36393f)
    .setDescription(`quazzer Hoş geldin kanalı ${kanal} olarak ayarlandı!`)
    .setThumbnail(client.user.avatarURL())
    .setFooter(`quazzer`);
